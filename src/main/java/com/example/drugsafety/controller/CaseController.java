...
import com.example.drugsafety.repository.CasePriorityRepository;
...
public class CaseController {
	...
	@RequestMapping(value = "/case-priority/new", method = RequestMethod.POST)
	@Transactional
	public ResponseModel addNewCasePriority(@Valid CaseCategory data) {
		this.entityManager.persist(data);
		this.entityManager.flush();
		return this.responseService.response(data);
	}

	@RequestMapping(value = "/case-priority/{id}", method = RequestMethod.PUT)
	@Transactional
	public ResponseModel updateCasePriority(@PathVariable("id") String id, @Valid CaseCategory data) {
	...
	@RequestMapping(value = "/case-priority/{id}", method = RequestMethod.DELETE)
	public ResponseModel deleteCasePriorityById(@PathVariable("id") String id) {
		CaseCategory caseCategory = this.entityManager.find(CaseCategory.class, id);
		if (caseCategory == null) {
			return this.responseService.response().statusCode(HttpStatus.NOT_FOUND);
		}
	...
	public ResponseModel getAllCasePriorities() {
		return this.responseService.response(this.casePriorityRepository.findAll());
	}

	@RequestMapping(value = "/case-priority/{id}", method = RequestMethod.GET)
	public ResponseModel getUserById(@PathVariable("id") String id) {
		return this.responseService.response(this.casePriorityRepository.findById(id)).notFoundIfNull();
